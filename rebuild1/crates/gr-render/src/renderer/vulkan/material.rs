...
impl VulkanMaterial {
    ...
    fn upload(
        ...
        let texture_set =
            MaterialTextureSet::resolve(descriptor, textures, defaults, texture_flags)?;
...
impl DefaultMaterialTextures {
    ...
    fn create(
        ...
        let base_color = Self::upload_default(
            device,
            memory_properties,
            queue_family_index,
            queue,
        ...
        let normal = match Self::upload_default(
            device,
            memory_properties,
            queue_family_index,
            queue,
            ...
            Ok(texture) => texture,
            Err(error) => {
                base_color.destroy(device);
                return Err(error);
            }
        };
        let metallic_roughness = match Self::upload_default(
            device,
            memory_properties,
            queue_family_index,
            queue,
        ...
        let occlusion = match Self::upload_default(
            device,
            memory_properties,
            queue_family_index,
            queue,
            ...
            Ok(texture) => texture,
            Err(error) => {
                metallic_roughness.destroy(device);
                normal.destroy(device);
                base_color.destroy(device);
                return Err(error);
            }
        };
        let emissive = match Self::upload_default(
            device,
            memory_properties,
            queue_family_index,
            queue,
    ...
    fn texture(&self, slot: MaterialTextureSlot) -> &VulkanTexture {
        match slot {
