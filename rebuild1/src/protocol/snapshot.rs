...
#[derive(Clone, Debug)]
pub enum DrawPacket {
    DebugTriangle(DebugTriangleDraw),
    Mesh(RenderItemPacket),
}

impl DrawPacket {
    /// Creates the explicit debug triangle draw used until real mesh packets are wired.
    pub fn debug_triangle() -> Self {
        Self::DebugTriangle(DebugTriangleDraw::default())
    }

    /// Creates one mesh draw packet from validated render item handles.
    pub fn mesh(item: RenderItemPacket) -> Self {
        Self::Mesh(item)
    }

    /// Returns the draw packet kind name used by renderer trace logs.
    pub fn name(&self) -> &'static str {
        match self {
            Self::DebugTriangle(_) => "DebugTriangle",
            Self::Mesh(_) => "Mesh",
        }
    }
}
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct DebugTriangleDraw {
    tint: [f32; 4],
}

impl Default for DebugTriangleDraw {
    /// Creates the default untinted debug triangle draw.
    fn default() -> Self {
        Self {
...
impl DebugTriangleDraw {
    /// Creates a debug triangle draw and clamps invalid tint values into the visible range.
    pub fn new(tint: [f32; 4]) -> Self {
        Self {
            tint: tint.map(clamp_unit),
        }
    }

    /// Returns the RGBA tint copied into the debug triangle frame uniform.
...
impl FrameSnapshotBuilder {
    ...
    /// Adds one renderer-facing draw packet to the snapshot.
    pub fn add_draw(&mut self, draw: DrawPacket) -> &mut Self {
    ...
    pub fn add_render_item(&mut self, item: RenderItemPacket) -> &mut Self {
        self.draws.push(DrawPacket::mesh(item.clone()));
...
/// Clamps a floating-point protocol color channel into the normalized unit interval.
...
mod tests {
    ...
    // Verifies that debug draw tint cannot carry NaN or out-of-range color values.
    #[test]
    fn debug_triangle_tint_is_clamped() {
        let draw = DebugTriangleDraw::new([1.5, -1.0, f32::NAN, 0.25]);

        assert_eq!(draw.tint(), [1.0, 0.0, 0.0, 0.25]);
    }
