...
use crate::{
    ...
    protocol::{
        AssetHandle, DrawPacket, DropReason, FrameId, FramebufferReadback,
        FramebufferReadbackOptions, LoadedAsset, MessageEnvelope, NativeSurfaceHandle,
        NativeSurfacePlatform, NonZeroExtent, RendererCommand, RendererEvent, RendererInbox,
...
impl VulkanContext {
    ...
    fn present_frame(
        ...
        self.validate_draw_packets(&snapshot.draws);
    ...
    /// Validates asset-backed draw packets before Vulkan frame recording begins.
    fn validate_draw_packets(&self, draws: &[DrawPacket]) {
        for draw in draws {
            if let DrawPacket::Mesh(item) = draw {
                let valid = self.assets.can_draw(item.mesh, item.material);
                tracing::trace!(
                    mesh = item.mesh.raw(),
                    material = item.material.raw(),
                    valid,
                    "validated mesh draw packet assets"
                );
            }
        }
    }
...
impl VulkanDevice {
    ...
    fn upload_imported_materials(
        ...
        self.materials
            .upload_imported_materials(&self.device, &self.memory_properties, materials)
    }
    ...
    fn destroy(mut self) {
        ...
        self.debug_triangle.destroy(&self.device);
...
fn create_device_for_surface(
    ...
    let debug_triangle = match DebugTriangleResources::create(
        instance,
        &device,
        candidate.physical_device,
        frames.slot_count(),
        ...
        Err(error) => {
            ...
            materials.destroy(&device);
            frames.destroy(&device);
            // Safety: no device resources escape because debug triangle creation failed before
            // returning the logical device to the context.
            unsafe {
                device.destroy_device(None);
            }
