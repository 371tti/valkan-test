...
impl PassOutput {
    ...
    /// Creates one depth output written by a graph pass.
    pub(crate) fn depth_clear(resource: GraphResource) -> Self {
        Self {
            resource,
...
impl GraphPass {
    ...
    /// Returns the first color output for compatibility with the current Vulkan executor.
    pub(crate) fn color_output(&self) -> Option<&PassOutput> {
        self.writes
            .iter()
            .find(|output| output.state() == ResourceState::ColorAttachment)
    }
...
impl FrameGraphInitialStates {
    ...
    /// Returns the current state of the persistent shadow map.
    pub(crate) fn shadow_map(self) -> ResourceState {
        self.shadow_map
    }
...
impl FrameGraphPlan {
    ...
    pub(crate) fn standard_frame_with_readback(
        ...
        let mut builder = FrameGraphBuilder::new()
            ...
            .pass(
                GraphPass::new(SHADOW_PASS)
                    .write(PassOutput::depth_clear_store(GraphResource::ShadowMap)),
            )
            .pass(
                GraphPass::new(SCENE_PASS)
                    .read(PassInput::read(
                        GraphResource::ShadowMap,
                        ResourceState::ShaderRead,
                    ))
                    ...
                    .write(PassOutput::depth_clear(GraphResource::SceneDepth)),
            )
            .pass(
...
mod tests {
    ...
    fn standard_frame_graph_orders_dependencies() {
        ...
        assert_eq!(names, [SHADOW_PASS, SCENE_PASS, POST_PASS, PRESENT_PASS]);
    ...
    fn standard_frame_graph_uses_actual_initial_states() {
        ...
        assert_eq!(
            graph.final_state_for(GraphResource::ShadowMap),
