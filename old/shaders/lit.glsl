...
float shadow_visibility(vec3 world_pos, vec3 light, vec3 normal) {
    int shadow_flags = int(scene.shadow_params.w + 0.5);
    if (shadow_flags == 0 || scene.shadow_params.z <= 0.0) {
        return 1.0;
    }
    ...
    bool has_depth_shadow = (shadow_flags & 1) != 0;
    bool has_transparent_shadow = (shadow_flags & 2) != 0;
    float transparent_visibility = 1.0;
    if (has_transparent_shadow) {
        float transparent_opacity = texture(shadow_opacity_map, uv).r;
        transparent_visibility = 1.0 - transparent_opacity * scene.shadow_params.z * 0.72;
    }
    if (!has_depth_shadow) {
        return transparent_visibility;
    }
    ...
    float center_depth = texture(shadow_map, uv).r;
    if (receiver_depth + scene.shadow_params.y * 3.0 < center_depth) {
        return transparent_visibility;
    }
    float center_lit = sample_shadow_compare(uv, receiver_depth);
    float deep_shadow = receiver_depth - center_depth;
    if (center_lit < 0.5 && deep_shadow > scene.shadow_params.y * 12.0) {
        return min(1.0 - scene.shadow_params.z, transparent_visibility);
    }
    ...
    float penumbra = saturate((receiver_depth - center_depth) * scene.shadow_params.x * 0.32);
    float kernel_radius = mix(0.85, 2.65, penumbra) * mix(1.35, 0.82, light_alignment);
    float lit = center_lit * 2.0;
    ...
    float coarse_visibility = lit / weight_sum;
    if (coarse_visibility > 0.97 || coarse_visibility < 0.03 || kernel_radius < 1.05) {
        float opaque_visibility = mix(1.0 - scene.shadow_params.z, 1.0, coarse_visibility);
        return min(opaque_visibility, transparent_visibility);
    }
    for (int i = 4; i < 8; i++) {
        float weight = 1.0;
        lit += sample_shadow_compare(uv + poisson_disk[i] * texel * kernel_radius, receiver_depth) * weight;
    ...
    float opaque_visibility = mix(1.0 - scene.shadow_params.z, 1.0, lit / weight_sum);
