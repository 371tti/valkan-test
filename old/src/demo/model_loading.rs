...
pub fn load_scene_model(renderer: &mut Renderer) -> Option<ModelId> {
    let model = load_first_model(renderer, &model_paths());
    if model.is_none() {
        log::warn!("no scene model loaded; scene will render without a model");
    }
    model
}
