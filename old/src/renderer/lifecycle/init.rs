...
const SHADOW_OPAQUE_FRAG_SPV: &[u8] = include_bytes!("../../../shaders/shadow_opaque.frag.spv");
const SHADOW_CUTOUT_FRAG_SPV: &[u8] = include_bytes!("../../../shaders/shadow_cutout.frag.spv");
const SHADOW_TRANSPARENT_FRAG_SPV: &[u8] =
    include_bytes!("../../../shaders/shadow_transparent.frag.spv");
...
const SHADOW_OPAQUE_FRAG_SPV_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/shaders/shadow_opaque.frag.spv"
);
const SHADOW_CUTOUT_FRAG_SPV_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/shaders/shadow_cutout.frag.spv"
);
const SHADOW_TRANSPARENT_FRAG_SPV_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/shaders/shadow_transparent.frag.spv"
);
    ...
    pub fn new(window_ref: Arc<Window>) -> Self {
        ...
        let scene_bindings = SceneBindings::new(
            ...
            SceneImageDescriptors {
                ...
                shadow_opacity: shadow_map.opacity_descriptor(),
        ...
        let push_constants = vec![vk::PushConstantRange {
            stage_flags: vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT,
            offset: 0,
            size: mem::size_of::<ObjectPush>() as u32,
        }];

        let pipeline_desc = PipelineDesc::new(ShaderSet::watched_graphics(
            "lit_mesh",
            VERT_SPV_PATH,
            VERT_SPV,
            FRAG_SPV_PATH,
        ...
        let opaque_shadow_pipeline_desc = PipelineDesc::new(ShaderSet::watched_graphics(
            "opaque_shadow_map",
            VERT_SPV_PATH,
            VERT_SPV,
            SHADOW_OPAQUE_FRAG_SPV_PATH,
            SHADOW_OPAQUE_FRAG_SPV,
        ))
        .with_vertex_layout(ModelVertex::layout())
        .with_layout(
            vec![scene_bindings.layout, assets.texture_set_layout()],
        ...
        .with_depth(DepthConfig::default());
        let cutout_shadow_pipeline_desc = PipelineDesc::new(ShaderSet::watched_graphics(
            "cutout_shadow_map",
            VERT_SPV_PATH,
            VERT_SPV,
            SHADOW_CUTOUT_FRAG_SPV_PATH,
            SHADOW_CUTOUT_FRAG_SPV,
        ))
        .with_vertex_layout(ModelVertex::layout())
        .with_layout(
            vec![scene_bindings.layout, assets.texture_set_layout()],
        ...
        .without_color_attachment()
        .with_depth(DepthConfig::default());
        let transparent_shadow_pipeline_desc = PipelineDesc::new(ShaderSet::watched_graphics(
            "transparent_shadow_opacity",
            VERT_SPV_PATH,
            VERT_SPV,
            SHADOW_TRANSPARENT_FRAG_SPV_PATH,
            SHADOW_TRANSPARENT_FRAG_SPV,
        ))
        .with_vertex_layout(ModelVertex::layout())
        .with_layout(
            vec![scene_bindings.layout, assets.texture_set_layout()],
        ...
        .with_rasterization(RasterizationConfig::double_sided())
        .with_color_blend(ColorBlendConfig::additive())
        .with_depth(DepthConfig {
            write: false,
        ...
        let opaque_shadow_pipeline = build_pipeline_slot(
            &logical_device,
            pipeline_cache,
            swapchain.format,
            opaque_shadow_pipeline_desc,
            "opaque shadow pipeline",
        );
        let cutout_shadow_pipeline = build_pipeline_slot(
            &logical_device,
            pipeline_cache,
            swapchain.format,
            cutout_shadow_pipeline_desc,
            "cutout shadow pipeline",
        );
        let transparent_shadow_pipeline = build_pipeline_slot(
            &logical_device,
            pipeline_cache,
            ShadowMap::OPACITY_FORMAT,
            transparent_shadow_pipeline_desc,
            "transparent shadow pipeline",
        );
        ...
        Self {
            ...
            assets,
            pipelines,
            opaque_shadow_pipeline,
            cutout_shadow_pipeline,
            transparent_shadow_pipeline,
            post_pipeline,
            camera_meter,
