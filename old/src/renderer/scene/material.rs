...
impl MaterialTextures {
    ...
    pub const fn slots(&self) -> [Option<TextureId>; 5] {
        [
            self.base_color,
    ...
    pub fn remapped(self, textures: &[TextureId]) -> Self {
        Self {
            base_color: remap_texture(self.base_color, textures),
            metallic_roughness: remap_texture(self.metallic_roughness, textures),
            normal: remap_texture(self.normal, textures),
            occlusion: remap_texture(self.occlusion, textures),
            emissive: remap_texture(self.emissive, textures),
        }
    }
}
...
impl Material {
    ...
    pub fn texture_slot_count(&self) -> usize {
        self.textures.slots().into_iter().flatten().count()
    }

    pub fn descriptor_textures(&self) -> [TextureId; 5] {
        [
            self.base_color_texture().unwrap_or(TextureId::DEFAULT),
            self.metallic_roughness_texture()
                .unwrap_or(TextureId::DEFAULT),
            self.normal_texture().unwrap_or(TextureId::NORMAL),
            self.occlusion_texture().unwrap_or(TextureId::DEFAULT),
            self.emissive_texture().unwrap_or(TextureId::DEFAULT),
        ]
    }
    ...
    pub fn with_remapped_textures(mut self, textures: &[TextureId]) -> Self {
        self.textures = self.textures.remapped(textures);
        self
    }
    ...
    pub fn casts_depth_shadow(&self) -> bool {
        !self.is_translucent() || self.alpha_cutoff() > 0.0
    }

    pub fn casts_transparent_shadow(&self) -> bool {
        self.alpha_cutoff() <= f32::EPSILON
            && (self.alpha.is_blend() || self.base_color[3] < 0.999 || self.transmission > 0.001)
            && self.base_color[3] > 0.01
    }

    pub fn casts_shadow(&self) -> bool {
        self.casts_depth_shadow() || self.casts_transparent_shadow()
    }
}
...
fn remap_texture(texture: Option<TextureId>, textures: &[TextureId]) -> Option<TextureId> {
    texture.and_then(|texture| textures.get(texture.0).copied())
}
