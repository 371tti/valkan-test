...
use pass_schedule::{PassSchedule, PassUpdates};
...
pub struct Renderer {
    ...
    opaque_shadow_pipeline: PipelineSlot,
    cutout_shadow_pipeline: PipelineSlot,
    transparent_shadow_pipeline: PipelineSlot,
...
fn log_model_stats(path: &Path, model: &CpuModel) {
    let texture_slots = model
        .primitives
        .iter()
        .map(|primitive| primitive.material.texture_slot_count())
        .sum::<usize>();
